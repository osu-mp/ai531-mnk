...
max_mcts_loops = 2000      # TODO: this should obviously be higher, low for debug
...
class Node:
    ...
    def expand(self):
        squares = self.board.get_empty_squares()
        for square in squares:
            node = Node(self.board, self, square)
...
def playout(node):
    ...
    while empty_squares:
        ...
        if board.is_gameover(selected_square, node.player):
...
def mcts(board: Board, player1, policy=None):
    """
    Simulate nodes from given board (until end of game)
    Return best action
    :param board:
    :param player1:
    :param policy:
