...
#
#
# # Player retrieved from the file
# def get_player():
...
def alphaBeta(position: Board, depth, alpha, beta, player, previousMove, first_player):
    ...
    for i in position.get_empty_squares():
        positionCopy = deepcopy(position)  # type: Board
        positionCopy.make_move(i, player)
        # recursive call with a deepcopy of the position, opposing player, and the updated move or i
        val = alphaBeta(positionCopy, depth - 1, alpha, beta, get_other_player(player), i, first_player)
        # If player is alpha
